...
詳情面板分成「每日／趨勢」兩頁。「每日」可用前一天／後一天瀏覽最近 30 個
日曆日；選定日期會切換當日總時長、應用排行與 AI 工作摘要，點中央日期可回到
今日。24 小時直方圖刻意只顯示今日；1.3.0 起保存的歷史時段分布用於「趨勢」
熱力圖。沒有紀錄的日期會顯示明確空狀態，不推測數值。

「趨勢」顯示最近 7 個完整日中**有紀錄日期**的平均（附 `N/7` 覆蓋率與前一期
比較）、缺日不冒充零值的每日柱狀／30 日曲線，以及最近 28 個完整日的
星期×時段熱力圖。每日 `hours[24]` 從 1.3.0 起才保存；舊歷史仍能參與每日平均，
但不會被假裝成有時段明細，熱力圖會顯示目前累積的 `N/28` 天覆蓋率。

面板另含 AI 工作時長區塊（今日聯集時長、
「N 個工作中」即時狀態、並行峰值與合計、7 天 AI 柱狀圖）、昨日比較、今日
24 小時直方圖（當前小時高亮）、選定日期的應用排行、完整日趨勢與熱力圖。
圖表全部使用 Material You 單色階（`colPrimary`，換壁紙自動變）；hover 任一柱、
點或熱力格會在圖上方讀出數值，熱力圖也支援鍵盤方向鍵瀏覽。
Esc 或點擊外部關閉。
...
在側欄進入磁貼編輯模式後，`Screen time` 會出現在「未使用」區；點一下即可加入，
停用後也能用同一方式加回。需要直接管理設定時，也可把下列項目加入
...
stock 的未使用清單原本只從寫死的 `availableToggleTypes` 產生；本模塊的第二個
insert-only 補丁會在未使用區補上自己的列，因此不需要為首次加入或重新加入手改
JSON。
