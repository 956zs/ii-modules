...
手機風格的焦點使用時間統計，提供 Daily 與 Weekly 兩個詳情頁、30 天可瀏覽歷史、
...
- 本週總時長與七日柱狀圖包含今日即時資料，未到的日期保持空白。
- 與上週同一批星期比較總時長及各應用變化，避免週中拿兩天去比較上週七天。
- 顯示本週使用最多的應用、使用時長與本週應用排行。
...
不會把不完整時段冒充完整日。週報加總的是這些 daily records 與今日即時資料，
因此每天被摺疊的低用量應用會合併到「其他」。
...
- 週報只比較本週截至今日與上週同一批星期；任一側缺日就不產生差異數字。
