...
PopupWindow {
    ...
    readonly property real targetPanelHeight: Math.min(stackView.implicitHeight + 32,
        root.maxPanelHeight)
    ...
    function deviceName(text, prefix) {
        ...
        for (let i = 0; i < prefix.length; i++) {
            while (source[cursor] === "_"
                    && source[cursor + 1]?.toLocaleLowerCase() === prefix[i].toLocaleLowerCase())
                cursor++
            if (source[cursor]?.toLocaleLowerCase() !== prefix[i].toLocaleLowerCase())
                return Translation.tr("Unknown device")
            cursor++
        }
        const name = source.slice(cursor).trim()
        return name !== "" ? name : Translation.tr("Unknown device")
    }
    ...
    component BluetoothPanel: ColumnLayout {
        ...
        readonly property var connectedDevices: {
            ...
            for (const disconnect of grouped.disconnects) {
                ...
                disconnectCounts[key] = (disconnectCounts[key] ?? 0) + 1
            }
            for (const profile of grouped.profiles) {
                const key = root.normalizedDevice(profile.text, "Audio Profiles for ")
            ...
            for (const disconnect of grouped.disconnects) {
                const key = root.normalizedDevice(disconnect.text, "Disconnect ")
                const profile = disconnectCounts[key] === 1 && profileCounts[key] === 1
                    ? grouped.profiles.find(candidate => root.normalizedDevice(
                        candidate.text, "Audio Profiles for ") === key) ?? null
                    : null
                devices.push({
