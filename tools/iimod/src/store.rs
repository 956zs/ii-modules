...
pub fn store_payload(id: &str, version: &str, payload: &Path) -> Result<PathBuf> {
    ...
    write_qmldir(id, &dest)?;
    Ok(dest)
}

/// qmldir registering the payload's uppercase-named components, or None when
/// it exposes no types. Without a qmldir, `import qs.mod.<id>` only resolves
/// in an engine that already registered the directory (the shell via
/// ModuleImports) — the standalone settings app could not load any module's
/// settings fragment.
pub fn qmldir_content(id: &str, dir: &Path) -> Result<Option<String>> {
    let mut types: Vec<String> = std::fs::read_dir(dir)?
        .flatten()
        .filter_map(|e| {
            let name = e.file_name().to_string_lossy().into_owned();
            let stem = name.strip_suffix(".qml")?;
            stem.chars()
                .next()
    ...
    types.sort();
    let mut out = format!("# GENERATED by iimod — do not edit.\nmodule qs.mod.{id}\n");
    for t in &types {
        out.push_str(&format!("{t} 1.0 {t}.qml\n"));
    }
    Ok(Some(out))
}

/// Generate (or remove a stale) qmldir inside an installed/staged payload dir.
pub fn write_qmldir(id: &str, dir: &Path) -> Result<()> {
    match qmldir_content(id, dir)? {
        Some(content) => std::fs::write(dir.join("qmldir"), content)?,
        None => {
            let _ = std::fs::remove_file(dir.join("qmldir"));
        }
    }
    Ok(())
}
