...
pub const BASENAME_DENYLIST_BASELINE: &[&str] = &[
    "Config.qml", "Appearance.qml", "Directories.qml", "Persistent.qml",
    "GlobalStates.qml", "Network.qml", "Translation.qml", "ResourceUsage.qml",
    "NetworkTraffic.qml", "Battery.qml", "Notifications.qml", "StyledText.qml",
    "MaterialSymbol.qml", "StyledPopup.qml", "Graph.qml", "Wallpapers.qml",
];
...
pub fn is_valid_locale(key: &str) -> bool {
    ...
    let Some(us) = key.find('_') else { return false };
...
fn validate_localized(field: &str, m: &LocalizedString, max_en: Option<usize>) -> Result<()> {
    let Some(en) = m.get("en_US") else {
        return Err(bail(exit::VALIDATION, format!("{field}: en_US key is required")));
    };
    if en.trim().is_empty() {
        return Err(bail(exit::VALIDATION, format!("{field}.en_US: must be non-empty")));
    }
    if let Some(max) = max_en {
        if en.chars().count() > max {
            return Err(bail(exit::VALIDATION, format!("{field}.en_US: exceeds {max} chars")));
        }
    }
    for key in m.keys() {
        if !is_valid_locale(key) {
            return Err(bail(exit::VALIDATION, format!("{field}: invalid locale key {key:?}")));
        }
    }
    Ok(())
}
...
pub fn validate(manifest: &Manifest) -> Result<Vec<String>> {
    ...
    if !manifest.license.chars().all(|c| c.is_ascii_alphanumeric() || ".+-".contains(c)) {
        warnings.push(format!("license {:?} does not look like an SPDX token", manifest.license));
    }
    if let Some(hp) = &manifest.homepage {
        if !(hp.starts_with("http://") || hp.starts_with("https://")) {
            return Err(bail(exit::VALIDATION, "homepage: must start with http:// or https://"));
        }
    }
    ...
    for (label, entry) in [
        ...
        if let Some(e) = entry {
            if !is_safe_rel_path(e) || !e.ends_with(".qml") {
                return Err(bail(exit::VALIDATION, format!("{label}: must be a safe relative .qml path")));
            }
        }
    }
    ...
    for (i, probe) in manifest.compat.probes.iter().enumerate() {
        ...
        if !is_safe_rel_path(&probe.path) || probe.path.starts_with("mod/") {
            return Err(bail(exit::VALIDATION, format!("{ctx}.path: must be a safe path under $II, not under mod/")));
        }
        match probe.probe_type {
            ProbeType::FileContains => {
                let Some(pattern) = &probe.pattern else {
                    return Err(bail(exit::VALIDATION, format!("{ctx}: file-contains requires pattern")));
                };
                if pattern.chars().count() < 6 {
                    return Err(bail(exit::VALIDATION, format!("{ctx}.pattern: minimum 6 characters")));
                }
                if pattern.contains('\n') {
                    return Err(bail(exit::VALIDATION, format!("{ctx}.pattern: must be single-line")));
                }
            }
            ProbeType::FileExists => {
                if probe.pattern.is_some() {
                    return Err(bail(exit::VALIDATION, format!("{ctx}: file-exists takes no pattern")));
                }
            }
        }
        if probe.reason.trim().is_empty() {
            return Err(bail(exit::VALIDATION, format!("{ctx}.reason: must be non-empty")));
        }
    }
    if let Some(t) = &manifest.compat.tested_on {
        if let Some(c) = &t.dots_commit {
            let ok = (7..=40).contains(&c.len()) && c.chars().all(|ch| ch.is_ascii_hexdigit() && !ch.is_ascii_uppercase());
            if !ok {
                return Err(bail(exit::VALIDATION, "compat.testedOn.dotsCommit: must be 7-40 lowercase hex chars"));
            }
        }
    }
    ...
    for (i, dep) in manifest.requires.modules.iter().enumerate() {
        ...
        if dep_ids.contains(&dep.id) {
            return Err(bail(exit::VALIDATION, format!("{ctx}: duplicate dependency id")));
        }
    ...
    for (i, s) in manifest.requires.system.iter().enumerate() {
        if s.bin.is_empty() || s.bin.contains('/') {
            return Err(bail(exit::VALIDATION, format!("requires.system[{i}].bin: must be a bare binary name")));
        }
        if s.hint.trim().is_empty() {
            return Err(bail(exit::VALIDATION, format!("requires.system[{i}].hint: must be non-empty")));
        }
    }

    for (i, c) in manifest.conflicts.iter().enumerate() {
        if !is_valid_id(c) {
            return Err(bail(exit::VALIDATION, format!("conflicts[{i}]: invalid id")));
        }
        if *c == manifest.id {
            return Err(bail(exit::VALIDATION, format!("conflicts[{i}]: self-conflict")));
        }
    }
    ...
    for (i, patch) in manifest.patches.iter().enumerate() {
        ...
        if patch.file.starts_with("mod/") || patch.file == "mod" {
            return Err(bail(exit::VALIDATION, format!("{ctx}.file: patches may not target mod/")));
        }
        if !patch.file.ends_with(".qml") {
            return Err(bail(exit::VALIDATION, format!("{ctx}.file: only .qml stock files may be patched in protocolVersion 1")));
        }
        if patch.anchor.chars().count() < 10 {
            return Err(bail(exit::VALIDATION, format!("{ctx}.anchor: minimum 10 characters")));
        }
        if patch.anchor.contains('\n') {
            return Err(bail(exit::VALIDATION, format!("{ctx}.anchor: must be single-line")));
        }
        if patch.content.is_empty() {
            return Err(bail(exit::VALIDATION, format!("{ctx}.content: must be non-empty")));
        }
        if patch.content.contains('\r') || patch.content.contains('\0') {
            return Err(bail(exit::VALIDATION, format!("{ctx}.content: must be LF-only UTF-8 without NUL")));
        }
        if patch.content.lines().count() > 200 {
            return Err(bail(exit::VALIDATION, format!("{ctx}.content: exceeds 200 lines")));
        }
        for line in patch.content.lines() {
            if FENCE_MARKER_HINTS.iter().any(|m| line.contains(m)) {
                return Err(bail(exit::VALIDATION, format!("{ctx}.content: contains fence-marker-like line (injection guard)")));
            }
        }
    }
...
mod tests {
    ...
    fn slot_entry_and_probe_rules() {
        ...
        expect_code(mutate("valid/tier-b-full.json", |v| {
